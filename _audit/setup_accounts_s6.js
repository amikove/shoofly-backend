...
async function main() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const email = `s6_oeil${i}_${TS}_${rand}@test.local`;
    const phone = `05${String(TS).slice(-6)}${rand}`.slice(0, 10);
    let r = await api('POST', '/auth/register', {
      email, password: PASS, first_name: `S6Oeil${i}`, last_name: 'Scenario',
      role: 'oeil', phone, city: 'Rabat', quartier: 'Agdal',
    });
    if (r.status !== 201) throw new Error(`s6_oeil${i} register failed: ` + JSON.stringify(r));
    ...
    oeils6.push({ id: oeilId, token: r.data.token, email, phone });
  }
  accounts.oeils6 = oeils6;
  fs.writeFileSync(path.join(__dirname, 'accounts_s.json'), JSON.stringify(accounts, null, 2));
  console.log('OK — 6 Œils frais ajoutés (accounts.oeils6)');
  await end();
}
